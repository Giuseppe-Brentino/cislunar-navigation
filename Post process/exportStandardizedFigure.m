...
for k = 1:length(f.Children)                         % for each subfigure
    ...
    elseif isa(ax, 'matlab.graphics.axis.Axes') || isa(ax, 'matlab.graphics.axis.GeographicAxes')
        ...
        for j = 1:length(ax.Children)
            ...
            elseif changeColors && isa(ax.Children(j),'matlab.graphics.chart.primitive.Bar')
                jHistograms = jHistograms+1;
                countColors = countColors + 1;
                histgrams(jHistograms) = ax.Children(j);
                indexHistColors = [indexHistColors countColors];
        ...
        existHistograms = ~isempty(indexHistColors);
        ...
        if existLines || existSurfaces || existScatters || existStairs || existHistograms
            ...
            %%% change colors
            if changeColors
                nIndexColors = Nlines + Nsurface + Nscatter + Nstair + Nhist;
                iLines = 0;
                iSurface = 0;
                iScatter = 0;
                iStair = 0;
                ...
                for i = 1:nIndexColors
                    ...
                    elseif any((nIndexColors+1-i)==indexHistColors)
                        iHist = iHist+1;
                        histgrams(Nhist+1-iHist).FaceColor = colors{index};
            ...
            for i = 1:Nlines
                ...
                lines(Nlines+1-i).LineWidth = mult;              % general settings
            end
        end
