...
[gm_rot, dgdx_rot] = SH.getDerivative(x_rot);
...
dg3b_dx = Propagation.Earth.mu.value * (3*(x_prev(1:3)-x_e)*(x_prev(1:3)-x_e)'...
    /norm(x_prev(1:3)-x_e)^5 - eye(3)./norm(x_prev(1:3)-x_e)^3);
STM(4:6,1:3) = dgdx + dg3b_dx;

dg3b_dx = Propagation.Earth.mu.value * (3*(x_prev(7:9)-x_e)*(x_prev(7:9)-x_e)'...
    /norm(x_prev(7:9)-x_e)^5 - eye(3)./norm(x_prev(7:9)-x_e)^3);
dg2b_dx = Propagation.Moon.mu.value * (3*(x_prev(7:9))*(x_prev(7:9))'...
    /norm(x_prev(7:9))^5 - eye(3)./norm(x_prev(7:9))^3);

STM(10:12,7:9) = dg2b_dx + dg3b_dx;
